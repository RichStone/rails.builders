...
`SessionReminderJob` runs every minute in production. Verified Active Builders and Facilitators receive a reminder 36 hours before a ready session by default, configurable from 1 to 168 hours. Each delivery checks current preferences, eligibility, and session timing. Missed runs catch up only before the session starts. A delivery record prevents repeat sends for the same Builder, session, and scheduled start; a changed start time permits a fresh reminder. Failed sends are reported and retried on the next run. As with other mail delivery, a process failure after the provider accepts an email but before its delivery record commits can cause a retry.
